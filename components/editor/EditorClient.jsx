...
yaar {
    ...
    karo {
        ...
        number choice = suno();
        agar (choice == 0) {
            attempts = 5; // Manually failing the 'attempts < 5' condition
        }
    } jabtak (attempts < 5);
    ...
    const statusEnumRef = useRef(null);
    ...
    useEffect(() => {
        const initWasm = async () => {
            try {
                ...
                await mod.default();
                vmRef.current = null;
                statusEnumRef.current = mod.WasmVMStatus;
    ...
    const handleVMStatus = (status) => {
        const WasmVMStatus = statusEnumRef.current;
        const fullOutput = vmRef.current.get_output();
        const newOutput = fullOutput.substring(lastOutputLengthRef.current);
        ...
        } else if (status === WasmVMStatus.Error) {
            setIsAwaitingInput(false);
    ...
    const provideInput = (input) => {
        ...
        const nextStatus = vmRef.current.provide_input(input);
        handleVMStatus(nextStatus);
    };

    const runCode = async () => {
        if (wasmStatus !== "online") {
        ...
        try {
            const mod = await import("@/pkg/compiler.js");
            vmRef.current = mod.init_vm(code);
            const status = vmRef.current.start();
            handleVMStatus(status);
            setExecTime(`${Math.round(performance.now() - start)}ms`);
        } catch (err) {
            setOutput(prev => prev + `BUILD FAILED:\n${err.message || err}`);
        }
            ...
            <div className="h-12 md:h-14 flex items-center justify-between px-3 md:px-6 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 transition-colors gap-2 md:gap-4">
                <div className="flex items-center space-x-1 md:space-x-4 min-w-0">
                    <div className="flex items-center space-x-0.5 md:space-x-1">
                        <Link href="/" className="flex items-center space-x-1 md:space-x-2 px-2 md:px-3 py-1.5 md:py-2 text-slate-500 hover:text-sky-500 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-md transition-all text-xs md:text-sm" title="Go Home">
                            <RiHome4Line className="w-4 h-4 md:w-5 md:h-5" />
                            <span className="hidden md:inline font-medium">Home</span>
                        </Link>
                        <Link href="/docs" className="flex items-center space-x-1 md:space-x-2 px-2 md:px-3 py-1.5 md:py-2 text-slate-500 hover:text-sky-500 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-md transition-all text-xs md:text-sm" title="Documentation">
                    ...
                    <div className={`hidden md:flex items-center space-x-2 px-2.5 py-1 rounded-md transition-all ${wasmStatus === 'online' ? 'bg-emerald-50 dark:bg-emerald-950/20 text-emerald-700 dark:text-emerald-400' : 'bg-amber-50 dark:bg-amber-950/20 text-amber-700 dark:text-amber-400'}`}>
                ...
                <div className="flex items-center space-x-0.5 md:space-x-2 flex-shrink-0">
                    ...
                    <button onClick={copyCode} className="p-1.5 md:p-2 text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-md transition-all" title="Copy Code">
                        {copied ? <RiCheckLine className="w-4 h-4" /> : <RiFileCopyLine className="w-4 h-4" />}
                    </button>
                    <button onClick={() => { setCode(""); setOutput(""); }} className="p-1.5 md:p-2 text-slate-500 hover:text-rose-600 dark:hover:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/20 rounded-md transition-all" title="Clear">
                        <RiDeleteBin6Line className="w-4 h-4" />
                    </button>
                    <div className="hidden md:block w-px h-6 bg-slate-200 dark:bg-slate-700"></div>
                    <button onClick={runCode} disabled={isCompiling} className={`flex items-center space-x-1 md:space-x-2 px-2.5 md:px-5 py-1.5 md:py-2 rounded-md font-semibold text-xs md:text-sm transition-all ${isCompiling ? 'bg-slate-100 dark:bg-slate-800 text-slate-400 cursor-not-allowed' : 'bg-sky-500 hover:bg-sky-600 text-white shadow-sm'}`}>
                        {isCompiling ? (
